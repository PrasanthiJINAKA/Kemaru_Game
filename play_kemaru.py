...
from funcs_kemaru import *
import time

COLORS=[(240,128,128), (255,160,122), (255,215,0), (238,232,170), (255,255,0), (173,255,47),
       (144,238,144), (152,251,152), (0,255,255), (224,255,255), (127,255,212), (135,206,250),
       (221,160,221), (255,105,180), (255,192,203), (245,245,220), (255,235,205), (255,250,205),
       (255,255,224), (255,228,181), (255,218,185), (255,228,225), (255,240,245), (245,255,250),
       (230,230,250), (240,255,240), (245,245,245), (220,220,220), (240,255,255), (32,178,170),
       (95,158,160), (0,191,255), (238,130,238)]
...
class Grid :
    def __init__(self, level):
        ...
        self.width = 540
        self.height = 540
        self.qn, self.ans, self.regions_map = random_grid(level)

        self.cell_list = [[Cell(i,j) for j in range(9)] for i in range(9)]
        self.vals = [[0 for j in range(9)] for i in range(9)]
        self.selected = None

        self.h_lines = get_h_borders(self.regions_map)
        self.v_lines = get_v_borders(self.regions_map)
        for row in range(9) :
            for col in range(9) :
                self.cell_list[row][col].val = self.qn[row][col]
                self.vals[row][col] = self.qn[row][col]
    ...
    def draw(self, win):
        ...
        for row_num, row in enumerate(self.h_lines):
            for col_num,thick in enumerate(row):
                pygame.draw.line(win, (0,0,255) if thick==5 else (128,128,128),(col_num*gap, row_num*gap),(col_num*gap + gap, row_num*gap) , thick)

        for row_num, row in enumerate(self.v_lines):
            for col_num,thick in enumerate(row):
                pygame.draw.line(win, (0,0,255) if thick==5 else (128,128,128), (col_num*gap, row_num*gap), (col_num*gap, row_num*gap + gap), thick)
    ...
    for i in range(9) :
        for j in range(9) :
            if board.cell_list[i][j].val == 0 :
                return None
    #sudoku matrix
    return(evaluate(board.vals, board.regions_map))
...
def PLAY(win) :
    ...
    while True :
        ...
        for row in board.ans :
            print(row)
        print("\n")
        for row in board.regions_map :
            print(row)
        print("\n")
        print ("H Lines")
        for row in board.h_lines:
            print (row)
        print ("\n")
        while gamerun :
            if not exit :
